...
import { FaGlobe, FaBoxOpen, FaCircleCheck, FaShip, FaPlane, FaLock, FaFileLines, FaTriangleExclamation, FaClipboardCheck, FaTruck, FaIndustry, FaFileSignature, FaBolt, FaShieldHalved, FaVial, FaHandshake, FaPhone } from 'react-icons/fa6'
import PageBanner from '../components/PageBanner'
import SeoMeta from '../components/SeoMeta'
      <SeoMeta
        title="Import and Export Chemicals | PTCGRAM Global Trade Services"
        description="PTCGRAM provides import and export services for industrial chemicals with compliant packaging, documentation support, and global logistics."
        canonical="/import-export"
        keywords="chemical export India, chemical import services, UN approved chemical packaging, export documentation chemicals"
      />
...
const SceneBanner = ({ scene, label }) => {
  const scenes = {
      ...
      <svg viewBox="0 0 800 320" className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
        <defs>
          <linearGradient id="sky" x1="0" y1="0" x2="0" y2="1">
            ...
            <stop offset="100%" stopColor="#293038"/>
          </linearGradient>
          <linearGradient id="sea" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="#3D464F"/>
        ...
        {/* Ship hull */}
        <rect x="120" y="170" width="560" height="60" rx="4" fill="#C7743D" opacity="0.9"/>
        <polygon points="680,170 740,230 120,230 80,170" fill="#884722"/>
        {/* Containers */}
        {[140,220,300,380,460,540].map((x,i) => (
          <g key={x}>
            <rect x={x} y={130} width="68" height="40" rx="2" fill={['#293038','#C7743D','#3D464F','#884722','#0B0F14','#D68A58'][i]} opacity="0.9"/>
      ...
      <svg viewBox="0 0 800 320" className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
        ...
        {[1,2,3].map(row => (
          [80,160,240,320,400,480,560,640,720].map((x,i) => (
            <g key={`${row}-${x}`}>
              <ellipse cx={x} cy={230 - (row-1)*75} rx="28" ry="10" fill={['#293038','#C7743D','#3D464F'][i%3]} opacity="0.9"/>
              <rect x={x-28} y={190 - (row-1)*75} width="56" height="40" fill={['#293038','#C7743D','#3D464F'][i%3]} opacity="0.85"/>
              <ellipse cx={x} cy={190 - (row-1)*75} rx="28" ry="10" fill={['#59636D','#D68A58','#7C8791'][i%3]}/>
        ...
        {/* MSDS label */}
        <rect x="350" y="260" width="100" height="30" rx="4" fill="#0B0F14" opacity="0.8"/>
    ...
    globe: (
      <svg viewBox="0 0 800 320" className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
        <rect width="800" height="320" fill="#0B0F14"/>
        {/* Grid lines */}
        {[80,160,240,320,400,480,560,640,720].map(x => (
          <line key={x} x1={x} y1="0" x2={x} y2="320" stroke="#293038" strokeWidth="0.5"/>
        ))}
        {[40,80,120,160,200,240,280].map(y => (
          <line key={y} x1="0" y1={y} x2="800" y2={y} stroke="#293038" strokeWidth="0.5"/>
        ))}
        {/* Route lines */}
        <path d="M200,160 Q400,80 600,140" stroke="#C7743D" strokeWidth="2" fill="none" strokeDasharray="6 3" opacity="0.8"/>
        <path d="M150,180 Q400,260 650,190" stroke="#C7743D" strokeWidth="1.5" fill="none" strokeDasharray="4 4" opacity="0.6"/>
        <path d="M300,100 Q450,200 700,160" stroke="#D68A58" strokeWidth="1.5" fill="none" strokeDasharray="4 4" opacity="0.5"/>
        {/* City dots */}
        {[
          ...
          <g key={name}>
            <circle cx={x} cy={y} r="8" fill="#C7743D" opacity="0.3"/>
            <circle cx={x} cy={y} r="4" fill="#C7743D"/>
            <circle cx={x} cy={y} r="4" fill="#C7743D" opacity="0.5" className="animate-ping"/>
      ...
      <section className="py-12 sm:py-14 lg:py-16 bg-white border-b border-gray-100">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            ...
            <StatCard icon={<FaCircleCheck />} value="UN" label="Approved Packaging" />
            <StatCard icon={<FaShip />} value="Sea · Air · Road" label="Shipping Modes" />
          </div>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 sm:gap-12 lg:gap-14 items-center">
            ...
            <div className="order-1 lg:order-2">
              ...
              <div className="grid grid-cols-1 gap-3">
                {[
                  [<FaLock key="lock" />, 'UN Approved Packaging', 'Hazardous chemicals packed per UN standards for safe international transport.'],
                  [<FaFileLines key="fumigation" />, 'Fumigation Certificate', 'Fumigation certification provided when required by the destination country.'],
                  [<FaTriangleExclamation key="dgd" />, 'Dangerous Goods Declaration', 'Full DGD documentation for regulated chemical shipments.'],
                  [<FaClipboardCheck key="msds" />, 'MSDS Documentation', 'Material Safety Data Sheets provided with every shipment.'],
                  [<FaBoxOpen key="packaging" />, 'Custom Export Packaging', 'Tailored packaging solutions to meet buyer and regulatory specifications.'],
                ].map(([icon, title, desc]) => (
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
                {[
                  ...
                  [<FaTruck key="road" />, 'Road Transport', 'Pan-India distribution'],
                ].map(([icon, mode, desc]) => (
      ...
      <section className="py-14 sm:py-16 lg:py-20 bg-white border-t border-gray-200">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              [<FaIndustry key="manufacturer" />, 'Direct Manufacturer Access', 'Strong relationships with Indian manufacturers for competitive pricing and quality assurance.'],
              [<FaFileSignature key="documentation" />, 'Full Documentation Support', 'We handle all trade documentation from DGD to Certificate of Origin — end to end.'],
              [<FaBolt key="turnaround" />, 'Fast Turnaround', 'Efficient processing, ready stock, and established freight relationships ensure quick dispatch.'],
              [<FaShieldHalved key="compliance" />, 'Regulatory Compliance', 'All shipments comply with Indian export regulations, destination country requirements, and IATA/IMO rules.'],
